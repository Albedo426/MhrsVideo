...
class DefaultGlVideoFrameDrawer() : GlVideoFrameDrawer {
    ...
    private class I420BufferTextureUploader {
        ...
        fun uploadFromBuffer(buffer: VideoFrameI420Buffer): IntArray {
            val strides = arrayOf(buffer.strideY, buffer.strideU, buffer.strideV)
            val planes = arrayOf(buffer.dataY, buffer.dataU, buffer.dataV)
            return uploadYuvData(buffer.width, buffer.height, strides.toIntArray(), planes)
        }
        ...
        private fun uploadYuvData(
            ...
            val planeWidths = arrayOf(width, width / 2, width / 2)
            val planeHeights = arrayOf(height, height / 2, height / 2)
